...
- Shipped hardening follow-up (2026-04-27, malformed-json nested boundary): malformed JSON-like payload coverage now includes top-level and nested object/array boundary cases, asserting quoted `"stdout"`/`"stderr"` terms inside prose string content stay informative while true malformed key-like output fields (including nested output keys) still collapse.
...
- Focused verification stays cheap and deterministic:
  ...
  - `node --experimental-strip-types --test extensions/openclaw-mem/toolResultPersistE2E.test.mjs` (includes plain + structured JSON stdout/stderr collapse, escaped output-key docs negative-control, malformed-json top-level+nested boundary assertions, no-leak assertions, and benign JSON/docs non-overblock assertions)
