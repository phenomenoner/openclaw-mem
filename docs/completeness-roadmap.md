...
6) **Safety hardening**
  ...
  - Shipped (2026-04-27, malformed-json fallback boundary): tool-result summary/runtime coverage now also asserts `{`-prefixed malformed JSON-like payloads that only mention quoted `"stdout"`/`"stderr"` inside prose string content stay informative in both top-level and nested object/array contexts, while true malformed key-like output fields (including nested output keys) still collapse.
   ...
   - Suggested focused verification commands:
    ...
    - `node --experimental-strip-types --test extensions/openclaw-mem/toolResultPersistE2E.test.mjs` (includes plain + structured JSON stdout/stderr collapse, malformed-JSON fallback boundary assertions for top-level + nested contexts, no-leak checks, and benign JSON/docs non-overblock assertions)
