...
- `optimize consolidation-review` scans `episodic_events` and emits dream-style **candidate** groups for summary compression, archive review, and cross-session link review — with provenance refs back to source episode IDs and recent-use protection when episode refs point at still-selected observations.
