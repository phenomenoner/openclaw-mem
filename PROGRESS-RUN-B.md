...
| T05 | done | pending commit | Added idempotent `init`, atomic fill-only `~/.openclaw-mem/config.toml`, capability receipts, and env > TOML > built-in resolution wired into DB, scope, vector, embedding, and pack defaults. Clean-HOME init/store/recall target slice: 92 passed; broad CLI/DB/embedding/vector/pack regression: 258 passed plus 55 subtests. |
