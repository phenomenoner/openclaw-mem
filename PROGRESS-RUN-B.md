...
| T08 | done | pending commit | Completed the installer matrix with Gemini CLI, Cursor, and Windsurf JSON adapters using current official paths plus `--config-path` overrides. Added `doctor --harness` pass/warn/fail diagnostics, executable/skill/config checks, repair commands, and seven ≤5-step quickstarts. Harness/CLI regression: 252 passed plus 26 subtests; focused final slice: 89 passed; direct missing-install doctor exit-1 smoke and strict MkDocs build passed. |
