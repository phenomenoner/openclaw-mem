...
| T07 | done | pending commit | Added unified `install --harness` orchestration and core detect/plan/apply/verify phases for claude-code, codex, openclaw, and generic. Writes are atomic, changed existing targets are timestamp-backed-up, JSON/managed-card unrelated content is preserved, dry-run is zero-write, and optional apply-time verify is supported. Adapter/legacy/CLI slice: 94 passed; direct CLI dry-run/apply/verify smoke passed. |
