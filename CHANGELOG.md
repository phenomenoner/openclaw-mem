...
- Keep explicit broad behavior available via `triggerMode=always`.
- Add counterfactual tests proving long skill/tool content does not qualify the hook by itself.
...
- `optimize evolution-review` now emits bounded `set_soft_archive_candidate` lifecycle candidates derived from `signals.soft_archive_candidates`; classifier posture stays low-risk/proposal-first with `safe_for_auto_apply=false` by default.
...
- Added `optimize canary-advisory`, a deterministic read-only advisory report for future canary cron that consumes posture/verifier receipts and emits per-feature `can_enable` / `monitor_only` / `not_ready` decisions for `soft_archive_canary`, `lifecycle_mvp`, and optimizer gates without changing defaults, Working Set state, or hard-delete posture.
