...
class TestCliM0(unittest.TestCase):
    ...
    def test_pack_trace_candidate_scores_and_caps_contract(self):
        conn = _connect(":memory:")
                ...
                json.dumps(
                    {
                        "ts": "2026-02-04T13:00:00Z",
                        "kind": "fact",
                        "summary": "score contract sample",
                        "summary_en": "score contract sample",
                        "tool_name": "memory_store",
                        "detail": {
                            "importance": "low",
                            "trust_tier": "standard",
                ...
                json.dumps(
                    {
                        "ts": "2026-02-04T13:01:00Z",
                        "kind": "fact",
                        "summary": "score contract sample two",
                        "summary_en": "score contract sample two",
                        "tool_name": "memory_store",
                        "detail": {
                            "importance": {"label": "nice to have", "score": 0.4},
                            "trust_tier": "trusted",
                        },
                    }
                ),
        ...
        args = build_parser().parse_args(["pack", "--query", "score", "--query-en", "score", "--trace", "--json", "--limit", "2"])
        ...
        for candidate in trace["candidates"]:
            ...
            self.assertIn("fts", scores)
            self.assertIn("semantic", scores)
            self.assertIsInstance(scores["rrf"], (int, float))
            self.assertIsInstance(scores["fts"], (int, float))
            self.assertIsInstance(scores["semantic"], (int, float))
            self.assertGreaterEqual(scores["rrf"], 0.0)
            self.assertGreaterEqual(scores["fts"], 0.0)
            self.assertGreaterEqual(scores["semantic"], 0.0)
            self.assertLessEqual(scores["rrf"], 1.0)
            self.assertLessEqual(scores["fts"], 1.0)
            self.assertLessEqual(scores["semantic"], 1.0)

            caps = candidate["decision"]["caps"]
            self.assertIn("niceCapHit", caps)
            self.assertIn("l2CapHit", caps)
            self.assertIsInstance(caps["niceCapHit"], bool)
            self.assertIsInstance(caps["l2CapHit"], bool)

        self.assertEqual(trace["output"]["l2IncludedCount"], 0)
        conn.close()

    def test_pack_trace_refreshed_refs_preserve_included_order(self):
        conn = _connect(":memory:")
                ...
                json.dumps(
                    {
                        "ts": "2026-02-04T13:00:00Z",
                        "kind": "fact",
                        "summary": "first order item",
                        "summary_en": "first order item",
                        "tool_name": "memory_store",
                        "detail": {},
                    }
                ),
                json.dumps(
                    {
                        "ts": "2026-02-04T13:01:00Z",
                        "kind": "fact",
                        "summary": "second order item",
                        "summary_en": "second order item",
                        "tool_name": "memory_store",
                        "detail": {},
                    }
                ),
                json.dumps(
                    {
                        "ts": "2026-02-04T13:02:00Z",
                        "kind": "fact",
                        "summary": "third order item",
                        "summary_en": "third order item",
                        "tool_name": "memory_store",
                        "detail": {},
                    }
                ),
        ...
        conn.execute(
            "INSERT INTO observation_embeddings (observation_id, model, dim, vector, norm, created_at) VALUES (?, ?, ?, ?, ?, ?)",
            (3, "text-embedding-3-small", 2, pack_f32([1.0, 0.0]), l2_norm([1.0, 0.0]), "2026-02-05T00:00:00Z"),
        )
        conn.commit()
        ...
        args = build_parser().parse_args(["pack", "--query", "order", "--trace", "--json", "--limit", "2", "--budget-tokens", "1200"])
        ...
        self.assertEqual(len(out["trace"]["output"]["refreshedRecordRefs"]), 2)
        self.assertEqual(out["trace"]["output"]["refreshedRecordRefs"], [
            out["items"][0]["recordRef"],
            out["items"][1]["recordRef"],
        ])
        self.assertEqual(out["trace"]["output"]["includedCount"], 2)
        self.assertEqual(out["trace"]["output"]["excludedCount"], 1)
        self.assertEqual(len(out["trace"]["candidates"]), 3)
        conn.close()
    ...
    def test_pack_trace_hybrid_scores_reflect_retriever_membership(self):
        conn = _connect(":memory:")
        ...
        pack_state = {
            ...
            "vec_ids": {2},
            "vec_en_ids": set(),
            "rrf_scores": {1: 0.91, 2: 0.72, 3: 0.53},
            "obs_map": {
                1: {
                    "summary": "fts-only row",
                    "summary_en": "fts-only row",
                    "kind": "fact",
                    "lang": "en",
                },
                2: {
                    "summary": "vector-only row",
                    "summary_en": "vector-only row",
                    "kind": "fact",
                    "lang": "en",
                },
                3: {
                    "summary": "fts-and-vector row",
                    "summary_en": "fts-and-vector row",
                    "kind": "fact",
                    "lang": "en",
                },
            },
        ...
        args = build_parser().parse_args(["pack", "--query", "hybrid", "--trace", "--json", "--limit", "2"])
        ...
        self.assertIn("matched_fts", first["decision"]["reason"])
        self.assertNotIn("matched_vector", first["decision"]["reason"])
        ...
        for cand in (first, second, third):
            self.assertIn("caps", cand["decision"])
            caps = cand["decision"]["caps"]
            self.assertIn("niceCapHit", caps)
            self.assertIn("l2CapHit", caps)
            self.assertIsInstance(caps["niceCapHit"], bool)
            self.assertIsInstance(caps["l2CapHit"], bool)

        for score in (first["scores"], second["scores"], third["scores"]):
            self.assertGreaterEqual(score["rrf"], 0.0)
            self.assertLessEqual(score["rrf"], 1.0)
            self.assertGreaterEqual(score["fts"], 0.0)
            self.assertLessEqual(score["fts"], 1.0)
            self.assertGreaterEqual(score["semantic"], 0.0)
            self.assertLessEqual(score["semantic"], 1.0)
    ...
    def test_pack_trace_missing_row_reason_is_reported(self):
        ...
        self.assertEqual(candidates[0]["decision"]["included"], False)
